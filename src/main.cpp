...
int main() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (show_playground_window) {
      ImGui::Begin(
          "Playground",
          &show_playground_window); // Pass a pointer to our bool variable (the
    ...
    if (show_knob_window) {
       ...
       **/
      ImGui::BeginGroup();
      if (ImGuiKnobs::Knob("Volume", &knob_value, -6.0f, 6.0f, 0.1f, "%.1fdB",
                           ImGuiKnobVariant_Tick, 100)) {
        // value was changed
      }
      ImGui::SameLine();
      if (ImGuiKnobs::Knob("Volume", &knob_value, -6.0f, 6.0f, 0.1f, "%.1fdB",
                           ImGuiKnobVariant_Dot, 100)) {
        // value was changed
      }
      ImGui::SameLine();
      if (ImGuiKnobs::Knob("Volume", &knob_value, -6.0f, 6.0f, 0.1f, "%.1fdB",
      ...
      ImGui::SameLine();
      if (ImGuiKnobs::Knob("Volume", &knob_value, -6.0f, 6.0f, 0.1f, "%.1fdB",
                           ImGuiKnobVariant_WiperOnly, 100)) {
        // value was changed
      }
      ImGui::SameLine();
      if (ImGuiKnobs::Knob("Volume", &knob_value, -6.0f, 6.0f, 0.1f, "%.1fdB",
                           ImGuiKnobVariant_WiperDot, 100)) {
        // value was changed
      }
      ImGui::SameLine();
      if (ImGuiKnobs::Knob("Volume", &knob_value, -6.0f, 6.0f, 0.1f, "%.1fdB",
                           ImGuiKnobVariant_Stepped, 100)) {
        // value was changed
      }
      ImGui::SameLine();
      if (ImGuiKnobs::Knob("Volume", &knob_value, -6.0f, 6.0f, 0.1f, "%.1fdB",
                           ImGuiKnobVariant_Space, 100)) {
        // value was changed
      }
      ...
      // Outline - BLUE
      draw_list->AddCircle(midpoint, 50,
                           ImGui::GetColorU32(IM_COL32(0, 0, 255, 255)), 0, 10);

      // Small filled circle for midpoint
      draw_list->AddCircleFilled(
          midpoint, 5, ImGui::GetColorU32(IM_COL32(255, 0, 255, 255)), 0);

      // Small filled circle on the above circular path
      draw_list->AddCircleFilled(
          point_on_circle, 3, ImGui::GetColorU32(IM_COL32(255, 0, 0, 255)), 0);

      // Line starting from midpoint to circumference of the circle
      draw_list->AddLine(midpoint, point_on_circle,
                         ImGui::GetColorU32(IM_COL32(255, 0, 0, 255)), 2);
