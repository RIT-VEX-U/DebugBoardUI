...
//  'io.ConfigFlags |= ImGuiConfigFlags_NoMouseCursorChange' (note: the resizing
...
//  2022-XX-XX: Platform: Added support for multiple windows via the
//  ImGuiPlatformIO interface. 2022-11-22: Perform a dummy glfwGetError() read
//  to cancel missing names with glfwGetKeyName(). (#5908) 2022-10-18: Perform a
//  dummy glfwGetError() read to cancel missing mouse cursors errors. Using
//  GLFW_VERSION_COMBINED directly. (#5785) 2022-10-11: Using 'nullptr' instead
//  of 'NULL' as per our switch to C++11. 2022-09-26: Inputs: Renamed
//  ImGuiKey_ModXXX introduced in 1.87 to ImGuiMod_XXX (old names still
//  supported). 2022-09-01: Inputs: Honor GLFW_CURSOR_DISABLED by not setting
//  mouse position. 2022-04-30: Inputs: Fixed
//  ImGui_ImplGlfw_TranslateUntranslatedKey() for lower case letters on OSX.
//  2022-03-23: Inputs: Fixed a regression in 1.87 which resulted in keyboard
//  modifiers events being reported incorrectly on Linux/X11. 2022-02-07: Added
...
void ImGui_ImplGlfw_KeyCallback(GLFWwindow *window, int keycode, int scancode,
  ...
  // Workaround: X11 does not include current pressed/released modifier key in
  // 'mods' flags. https://github.com/glfw/glfw/issues/1630
  if (int keycode_to_mod = ImGui_ImplGlfw_KeyToModifier(keycode))
    mods = (action == GLFW_PRESS) ? (mods | keycode_to_mod)
                                  : (mods & ~keycode_to_mod);
  ImGui_ImplGlfw_UpdateKeyModifiers(mods);
...
static void ImGui_ImplGlfw_CreateWindow(ImGuiViewport *viewport) {
  ...
  glfwWindowHint(GLFW_DECORATED,
                 (viewport->Flags & ImGuiViewportFlags_NoDecoration) ? false
                                                                     : true);
...
// per-platform workarounds here (FIXME-VIEWPORT: Implement same work-around for
// Linux/OSX!)
#if !GLFW_HAS_MOUSE_PASSTHROUGH && GLFW_HAS_WINDOW_HOVERED && defined(_WIN32)
static LRESULT CALLBACK WndProcNoInputs(HWND hWnd, UINT msg, WPARAM wParam,
                                        LPARAM lParam) {
  ImGui_ImplGlfw_Data *bd = ImGui_ImplGlfw_GetBackendData();
  if (msg == WM_NCHITTEST) {
...
static void ImGui_ImplGlfw_ShowWindow(ImGuiViewport *viewport) {
  ...
  // be included in GLFW 3.3 via a GLFW_FOCUS_ON_SHOW window attribute. See
  // https://github.com/glfw/glfw/issues/1189
